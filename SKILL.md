...
| 「主控端版本过低，被控端不再兼容此协议版本」（退出码 6） | **term 通道冷态报文，与版本基本无关**（2026-09-19 翻转实验实测：GUI 终端窗口开着时 term 通道才可用；关窗即冷，报此错；管理面不受影响） | **在主控端 GUI 里打开一次目标设备的终端窗口，再重试 CLI**（秒级恢复）。GUI 终端开着仍报错且两端版本确实不等时才升级较低一端 |
