...
- 被控端支持 Windows / macOS（Linux/移动端不支持，官方）；**实测范围仅 Windows 被控端**。锁屏进终端需系统账户验证（Windows 主控端发起需手动输密码）
- Node.js ≥ 20
...
| 「主控端版本过低，被控端不再兼容此协议版本」（退出码 6） | 多为 term 通道未激活，与版本无关 | **在主控端 GUI 打开该设备的终端窗口，再重试 CLI**（秒级恢复）；终端开着仍报错且两端版本确实不等时，升级版本旧的一端 |
