...
| term 管道通道 | 主控端 4.41.0.2311 × 已更新的被控端 | 2026-09-18 `exec` → `[终端] 主控端版本过低，被控端不再兼容此协议版本`（退出码 6） | ❌ 官方协议不兼容 → 升级主控端 |
...
| 错误码 1003 /「主控端版本过低，被控端不再兼容此协议版本」（退出码 6） | 被控端版本高于主控端，协议不兼容（实测：主控端 4.41.0.2311 × 已升级的被控端） | **升级主控端**：主界面右上角【更多】→【检查更新】，或【设置】→【常规】开启自动更新 |
| 同上，但客户端已提示「最新」 | 被控端可能在内测/灰度通道，版本比稳定版主控端更高 | 在被控端【关于】查看版本与主控端对比；用主控端 GUI 进 [设备详情]→[终端] 交叉验证（GUI 也报同样错即官方版本问题，与本项目无关） |
