...
| 「主控端版本过低，被控端不再兼容此协议版本」（退出码 6） | **首选解释：term 通道冷态报文，与版本无关**。2026-09-19 翻转实验实测：GUI 终端未开/刚关时 `exec` 稳定报此错（管理面 `list` 正常），GUI 打开终端窗口后立即恢复，关闭后又立即失效 | **第一处置：在主控端 GUI 里打开一次目标设备的终端窗口，再重试 CLI**（实测秒级恢复）。仅当 GUI 打开终端后仍报错且两端版本确实不等时，才按版本不匹配处理（升级较低一端） |
